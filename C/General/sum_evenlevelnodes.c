/*Help of this program we are learning how to Print Even level node in BST. Using Recursive method*/
...
void print_even(struct Tree*,int);
...
/* Help of this function show BST Even level data*/
void print_even(struct Tree*temp,int show){
  /* If temp Not NULL */
  if(temp)
  {
    print_even(temp->left_child,show+1);
    print_even(temp->right_child,show+1);
    if(show%2!=0){
      /* Print even level BST node */
      printf(" %d",temp->data);
    }
  }
}
